        ...
        if sys.argv[1].upper() == 'SCAN':
            ...
            if getAddonSetting('autodetect', sType=BOOL):
                ...
                for interface in interfaces:
                    if interface in ('127.0.0.1', '127.0.1.1', 'localhost'):
                        continue
                    host = interface
                    break
            ...
            if os.path.isfile(scannerfile):
                ...
                for net_device in net_devices:
                    ...
                    if 'name' in net_devices[net_device]:
                        ...
                        label = '{} ({})'.format(net_device, net_devices[net_device].get('name'))
                    else:
        ...
        with busy_dialog():
            for device in devices:
                ...
                if device['multichannel']:
                    device.update({'status': sd.send(device['ip'], sd.STATUS[device['channel']],
                                                     channel=device['channel'] + 1, timeout=5)})
                else:
        ...
        if len(_devlist) > 0:
            ...
            if _idx > -1:
                if strToBool(_devlist[_idx].getProperty('switchable')):
                    res = sd.send(_devlist[_idx].getProperty('ip'),
                                  sd.TOGGLE[int(_devlist[_idx].getProperty('channel'))])
                    writeLog('%s (%s) switched to %s' % (_devlist[_idx].getProperty('name'),
                                                         _devlist[_idx].getProperty('ip'), res))
                else:
                    writeLog('%s (%s) is not switchable' % (_devlist[_idx].getProperty('name'),
                                                            _devlist[_idx].getProperty('ip')))
                    notify(LS(30000), LS(30014) % (_devlist[_idx].getProperty('name')),
